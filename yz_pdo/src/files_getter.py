...
class FilesGetter_old(object):

    def __init__(self, path_dir: Path, suffix: tuple[str, ...] = tuple()):
        self._path_dir = path_dir
        self.suffix = set([sf.lower() for sf in suffix])
        self.validate = bool(self.suffix)
    ...
    @staticmethod
    def get_folders(*, path_dir: Path) -> Iterator[Path]:
        return (folder for folder in path_dir.iterdir() if folder.is_dir())

    @staticmethod
    def _get_validate_files(path_dir):
        files = FilesGetter._get_files(path_dir=path_dir)
        return (file for file in files if file.suffix.lower() in self.suffix)

    def get_all_files(self) -> Iterator[Path]:
        list_folder = [self._path_dir, ]
        while list_folder:
            check_dir = list_folder.pop()
            list_folder.extend(FilesGetter.get_folders(path_dir=check_dir))
            if self.validate:
                files = FilesGetter._get_validate_files(path_dir=check_dir)
            else:
                files = FilesGetter._get_files(path_dir=check_dir)
            for file in files:
                yield file
    ...
    def get_files(self):
        if self.validate:
            return self._get_validate_files()
        return FilesGetter._get_files(path_dir=self._path_dir)

    def get_file(self, name: str):
        for file in self.get_all_files(validate=False):
            if file.name == name:
                return file
        else:
            raise ValueError(f"{name} is not Found")
