...
class YzViewer(DBViewer):
    ...
    def view_part_tables(self, name_part: str, count: int = 1) -> dict[str, int]:
        """
        Выводит входящие таблички
        """
        result = self.get_tables_include(name_part)
        return self._calculate_result(result, count)

    def view_tree_yz(self, name_part: str, count_y: int = 1) -> None:
        """
        Отображает дерево узлов входящих в узел 
        """
    ...
    def view_tree_detail(self, name_part: str, count_d: int = 1) -> None:
        """
        Отображает дерево узлов и деталей входящих в узел
        """
    ...
    def view_tree_tables(self, name_part: str, count_t: int = 1) -> None:
        """
        Отображает дерево табличек входящих в узел
        """
    ...
    def view_master_for_yz(self, name_part: str) -> None:
        """
        Показывает в какие узлы входит узел
        """
    ...
    def view_master_for_detail(self, name_detail: str) -> None:
        """
        Показывает в какие узлы входит деталь
        """
    ...
    def view_master_for_table(self, name_table: str) -> None:
        """
        Показывает в какие узлы входит табличка
        """
    ...
    def collect_parts(self, name_part: str, count: int = 1) -> dict[str, int]:
        """
        Возвращает словарь суммы всех входящих узлов
        """
        self.get_product(name_part)
    ...
    def collect_details(self, name_part: str, count: int = 1) -> dict[str, int]:
        """
        Возвращает словарь суммы всех входящих деталей
        """
    ...
    def collect_tables(self, name_part: str, count: int = 1) -> dict[str, int]:
        """
        Возвращает словарь суммы всех входящих табличек
        """
    ...
    def collect_all(self, name_part: str, count: int = 1) -> dict[str, int]:
        """
        Возвращает словарь суммы всего входящего в узел
        """
        all_collection = defaultdict(int)
        parts_collection = self.collect_parts(name_part, count)
        for yz, count_yz in parts_collection.items():
            all_collection[yz] += count_yz
            for detail, count_dt in self.view_part_details(yz, count_yz).items():
                all_collection[detail] += count_dt
            for table, count_tb in self.view_part_tables(yz, count_yz).items():
                all_collection[table] += count_tb
